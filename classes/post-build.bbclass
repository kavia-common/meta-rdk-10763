do_postbuild_shell () {
 ...
 BUILD_STATS_LATEST=$(find tmp/buildstats -printf '%T+ %p\n' | sort -r | cut -d ' ' -f 2 | head -n 1 | cut -d '/' -f 3)
 BUILD_STATS_PATH="tmp/buildstats/${BUILD_STATS_LATEST}/build_stats"
 
 START_TIME=$(grep -ir "Build Started:" ${BUILD_STATS_PATH} | cut -d ":" -f 2)
 #START_TIME=`date -d @$START_TIME`
 DURATION=$(grep -ir "Elapsed time:" ${BUILD_STATS_PATH} | cut -d ":" -f 2)
